const data = {
  Work: {
    "Running late": [
      ...
      "Running a bit late, will join soon",
      "Running behind, almost there"
    ],
    "Need more time": [
      "Need more time, will update soon",
      "Still working on it"
    ]
  },
  Friend: {
  ...
  Client: {
    "Running late": [
      "Apologies, running slightly late"
    ]
  }
};
...
function renderCategories() {
  document.getElementById("categories").innerHTML =
    Object.keys(data)
      .map(cat => `<button onclick="selectCategory('${cat}',event)">${cat}</button>`)
      .join('');
}
...
function selectCategory(cat, e) {
  ...
  document.getElementById("situations").innerHTML =
    Object.keys(data[cat])
      .map(s => `<button onclick="selectSituation('${s}',event)">${s}</button>`)
      .join('');
}
...
function selectSituation(s, e) {
  ...
  document.querySelectorAll("#situations button")
    .forEach(btn => btn.classList.remove("active"));
...
function saveEmail() {
  ...
  if (!check) {
    alert("Confirm you're not a bot");
    return;
  }
